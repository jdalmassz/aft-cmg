...
<template>
  ...
  <div v-else class="layout">
    <aside class="side">
      ...
      <nav>
        <router-link to="/inicio" class="nav-link" active-class="act">📊 Dashboard</router-link>
        <router-link to="/activos" class="nav-link" active-class="act">📦 Inventario</router-link>
        <router-link v-if="user?.rol === 'admin'" to="/usuarios" class="nav-link" active-class="act">👥 Usuarios</router-link>
      </nav>
      <div class="side-foot">
        <div class="who">{{ user?.nombre || user?.username }}</div>
        <div class="role">{{ user?.rol === 'admin' ? 'Administrador' : 'Usuario' }}</div>
        <button class="btn sec sm" @click="logout">Salir</button>
      </div>
    </aside>
    <main class="content">
      <router-view />
...
.nav-link { color: #cbd5e1; padding: 9px 12px; border-radius: 8px; font-weight: 600; font-size: 13px; }
