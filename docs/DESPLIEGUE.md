...
Sin la clave, la app no falla: entra por `/?sso=nodisponible` y se usa el login
local. Con la clave puesta, el paso 1 del documento (`callback-token`) tiene que
contestar `200`; un `401` casi siempre es la clave leída como texto en vez de
